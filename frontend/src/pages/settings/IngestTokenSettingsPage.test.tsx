...
import IngestTokenSettingsPage from './IngestTokenSettingsPage';
import { NotifyProvider } from '../../notify/NotifyProvider';

vi.mock('../../api/ingestTokens', () => ({
...
const list = listIngestTokens as ReturnType<typeof vi.fn>;
const create = createIngestToken as ReturnType<typeof vi.fn>;
const revoke = revokeIngestToken as ReturnType<typeof vi.fn>;

function ts(over: Partial<IngestTokenSummary> = {}): IngestTokenSummary {
  return {
    id: 1,
    name: 'Pi',
...
describe('IngestTokenSettingsPage', () => {
  beforeEach(() => {
    list.mockReset();
    create.mockReset();
    revoke.mockReset();
  ...
  it('shows empty state when no tokens', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/Noch keine Tokens/)).toBeInTheDocument(),
    );
    expect(
      screen.getByRole('button', { name: /Ersten Token erzeugen/i }),
    ).toBeInTheDocument();
  });

  it('renders active and revoked tokens with status chips', async () => {
    list.mockResolvedValueOnce([
      ts({ id: 1, name: 'Pi', revoked: false }),
  ...
  it('reveals the plaintext exactly once on create', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByLabelText('Plaintext-Token')).toHaveTextContent(
        'tk_secret_value',
      ),
    );
  ...
  it('copies plaintext to clipboard on click', async () => {
    list.mockResolvedValue([]);
    create.mockResolvedValueOnce({
      id: 99,
      name: 'Neuer',
    ...
    // userEvent.setup() registriert seinen eigenen clipboard-Stub — daher
    // hier ueber die ursprueglich exposed clipboard-API patchen.
    ...
    try {
      ...
      await waitFor(() =>
        expect(screen.getByLabelText('Plaintext-Token')).toBeInTheDocument(),
      );

      // Patch nach userEvent.setup() erneut anwenden, damit userEvent's eigener
      // Clipboard-Stub nicht greift.
      Object.defineProperty(navigator, 'clipboard', {
        value: { writeText },
        configurable: true,
        writable: true,
      ...
      expect(writeText).toHaveBeenCalledWith('tk_secret_value');
    } finally {
