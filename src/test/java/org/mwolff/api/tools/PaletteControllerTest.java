...
class PaletteControllerTest {
    ...
    void shouldReturnJsonPalette() throws Exception {
        ...
        // When / Then
        mockMvc.perform(multipart("/api/tools/palette").file(upload))
    ...
    void shouldReturn400WhenFileMissing() throws Exception {
        mockMvc.perform(multipart("/api/tools/palette"))
    ...
    void shouldReturn400WhenCountTooLow() throws Exception {
        final MockMultipartFile upload = new MockMultipartFile(
                "file", "photo.png", MediaType.IMAGE_PNG_VALUE, "raw".getBytes());
        mockMvc.perform(multipart("/api/tools/palette").file(upload).param("count", "1"))
                .andExpect(status().isBadRequest());
    }
    ...
    void shouldReturn400WhenCountTooHigh() throws Exception {
        final MockMultipartFile upload = new MockMultipartFile(
                "file", "photo.png", MediaType.IMAGE_PNG_VALUE, "raw".getBytes());
        mockMvc.perform(multipart("/api/tools/palette").file(upload).param("count", "11"))
                .andExpect(status().isBadRequest());
    }
