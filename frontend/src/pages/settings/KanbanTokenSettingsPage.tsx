...
type LoadState =
  ...
  | { kind: 'ready'; tokens: KanbanTokenSummary[] };
...
export default function KanbanTokenSettingsPage(): JSX.Element {
  ...
  const [created, setCreated] = useState<CreatedKanbanToken | null>(null);
  const [toRevoke, setToRevoke] = useState<KanbanTokenSummary | null>(null);

  async function reload(): Promise<void> {
    try {
      const list = await listKanbanTokens();
  ...
  async function handleCreate(): Promise<void> {
    ...
    try {
      const result = await createKanbanToken(name);
      setCreateOpen(false);
      setCreated(result);
      await reload();
    } catch (e) {
  ...
  async function handleConfirmRevoke(): Promise<void> {
    if (!toRevoke) return;
    try {
      await revokeKanbanToken(toRevoke.id);
      setToRevoke(null);
  ...
  return (
    <Box>
      <Breadcrumbs sx={{ mb: 2 }}>
        ...
        <Typography color="text.primary">Kanban-Tokens</Typography>
      </Breadcrumbs>

      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
        <Typography variant="h4">Kanban-Tokens</Typography>
        <Button
          variant="contained"
          startIcon={<AddIcon />}
          onClick={openCreate}
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        Kanban-Tokens erlauben Werkzeugen wie dem Board-Adapter des 9-Schritt-Workflows (`tbx`),
        auf dein Kanban-Board zuzugreifen — ohne Keycloak-Login. Behandle sie wie Passwörter.
      </Typography>
      ...
      {state.kind === 'ready' && state.tokens.length === 0 && (
        ...
        >
          ...
          <Typography variant="body2">
            Erzeuge einen Token, um dem Board-Adapter (`tbx`) Zugriff auf dein Kanban-Board zu
            geben.
          </Typography>
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            <TextField
              ...
              autoFocus
              placeholder="z. B. Board-Adapter Laptop"
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            Der Token „{toRevoke?.name}” wird sofort deaktiviert. Werkzeuge, die ihn verwenden,
            bekommen ab sofort 401.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
