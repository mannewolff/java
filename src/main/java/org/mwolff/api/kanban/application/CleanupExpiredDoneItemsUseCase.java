...
/**
 * Iteriert alle User mit mindestens einem DONE-Item und archiviert deren abgelaufene Items (#327).
 * Die eigentliche Archivierung pro User erledigt {@link
 * ExpiredDoneItemsPerUserCleanup#archiveForUser(String)} — als eigene Bean, damit pro User eine
...
public class CleanupExpiredDoneItemsUseCase {
  ...
  /** Liefert die Summe aller archivierten Items über alle User — für Logging. */
  public int execute() {
    final List<String> users = items.distinctUsersWithDoneItems();
    int total = 0;
    for (final String userSub : users) {
      total += perUserCleanup.archiveForUser(userSub);
    }
    return total;
  }
}
