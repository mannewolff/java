...
class BackgroundRemovalControllerTest {
    ...
    void shouldReturnPngOnSuccess() throws Exception {
        ...
        MockMultipartFile upload = new MockMultipartFile(
                "file", "icon.png", MediaType.IMAGE_PNG_VALUE, "raw".getBytes());

        // When / Then
        mockMvc.perform(multipart("/api/tools/remove-background").file(upload))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.IMAGE_PNG))
                .andExpect(header().string("Content-Disposition", "attachment; filename=\"transparent.png\""))
                .andExpect(content().bytes(processed));
    }

    @Test
    void shouldReturn400WhenFileMissing() throws Exception {
        mockMvc.perform(multipart("/api/tools/remove-background"))
    ...
    void shouldReturn502WhenPythonServiceFails() throws Exception {
        ...
        willThrow(new PythonToolsException("upstream down"))
                .given(service).removeBackground(any());
        MockMultipartFile upload = new MockMultipartFile(
                "file", "x.png", MediaType.IMAGE_PNG_VALUE, "raw".getBytes());
