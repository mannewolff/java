...
class CropOgControllerTest {
  ...
  void shouldReturnJpegOnSuccess() throws Exception {
    ...
    final MockMultipartFile upload =
        new MockMultipartFile("file", "photo.jpg", MediaType.IMAGE_JPEG_VALUE, "raw".getBytes());
  ...
  void shouldReturn400WhenYOffsetAboveOne() throws Exception {
    final MockMultipartFile upload =
        new MockMultipartFile("file", "photo.jpg", MediaType.IMAGE_JPEG_VALUE, "raw".getBytes());
    mockMvc
        .perform(multipart("/api/tools/crop-og").file(upload).param("y_offset", "1.5"))
  ...
  void shouldReturn400WhenYOffsetBelowZero() throws Exception {
    final MockMultipartFile upload =
        new MockMultipartFile("file", "photo.jpg", MediaType.IMAGE_JPEG_VALUE, "raw".getBytes());
    mockMvc
        .perform(multipart("/api/tools/crop-og").file(upload).param("y_offset", "-0.1"))
        .andExpect(status().isBadRequest());
  }
  ...
  void shouldReturn400WhenQualityBelow50() throws Exception {
    final MockMultipartFile upload =
        new MockMultipartFile("file", "photo.jpg", MediaType.IMAGE_JPEG_VALUE, "raw".getBytes());
    mockMvc
        .perform(multipart("/api/tools/crop-og").file(upload).param("quality", "10"))
        .andExpect(status().isBadRequest());
  }
  ...
  void shouldReturn400WhenQualityAbove95() throws Exception {
    final MockMultipartFile upload =
        new MockMultipartFile("file", "photo.jpg", MediaType.IMAGE_JPEG_VALUE, "raw".getBytes());
    mockMvc
        .perform(multipart("/api/tools/crop-og").file(upload).param("quality", "100"))
        .andExpect(status().isBadRequest());
  }
  ...
  void shouldReturn400WhenXOffsetOutOfRange() throws Exception {
    final MockMultipartFile upload =
        new MockMultipartFile("file", "photo.jpg", MediaType.IMAGE_JPEG_VALUE, "raw".getBytes());
    mockMvc
        .perform(multipart("/api/tools/crop-og").file(upload).param("x_offset", "1.5"))
        .andExpect(status().isBadRequest());
  }
  ...
  void shouldReturn400WhenWidthBelowMin() throws Exception {
    final MockMultipartFile upload =
        new MockMultipartFile("file", "photo.jpg", MediaType.IMAGE_JPEG_VALUE, "raw".getBytes());
    mockMvc
        .perform(multipart("/api/tools/crop-og").file(upload).param("width", "100"))
        .andExpect(status().isBadRequest());
  }
  ...
  void shouldReturn400WhenHeightAboveMax() throws Exception {
    final MockMultipartFile upload =
        new MockMultipartFile("file", "photo.jpg", MediaType.IMAGE_JPEG_VALUE, "raw".getBytes());
    mockMvc
        .perform(multipart("/api/tools/crop-og").file(upload).param("height", "5000"))
        .andExpect(status().isBadRequest());
  }
  ...
  void shouldUseCustomDimensionsInDownloadFilename() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            header()
                .string("Content-Disposition", "attachment; filename=\"featured-1080x1080.jpg\""));
  }
  ...
  void shouldReturn502WhenPythonServiceFails() throws Exception {
    ...
    willThrow(new PythonToolsException("upstream down"))
        .given(service)
        .crop(any(), anyDouble(), anyDouble(), anyInt(), anyInt(), anyInt());
    final MockMultipartFile upload =
        new MockMultipartFile("file", "photo.jpg", MediaType.IMAGE_JPEG_VALUE, "raw".getBytes());
